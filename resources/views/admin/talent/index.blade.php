...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <div class="d-flex align-items-center gap-3">
        <form method="GET" action="{{ route('admin.talent.index') }}" class="d-flex gap-2 align-items-center" id="search-form">
            ...
            @if(request()->hasAny(['search', 'status', 'start_date', 'end_date']))
                <a href="{{ route('admin.talent.index') }}" class="btn btn-sm btn-outline-danger"><i class="fas fa-times"></i></a>
            @endif
        </form>
        <div class="btn-group">
            <a href="{{ route('admin.export', ['resource' => 'talent', 'format' => 'excel'] + request()->query()) }}" id="export-excel" class="btn btn-outline-success" title="Export Excel">
                <i class="fas fa-file-excel"></i>
            </a>
            <a href="{{ route('admin.export', ['resource' => 'talent', 'format' => 'pdf'] + request()->query()) }}" id="export-pdf" class="btn btn-outline-danger" title="Export PDF">
    ...
    <div class="card-body p-0">
        <table class="table table-striped table-hover mb-0">
            ...
            <tbody id="table-body">
                ...
                <tr>
                    <td>
                        <input type="checkbox" class="form-check-input row-checkbox" value="{{ $talent->id }}">
                    </td>
                    <td>
                        @if($talent->image)
                ...
                @empty
                <tr>
                    <td colspan="7" class="text-center py-4 text-muted">No talent found. Add one to get started!</td>
                </tr>
                @endforelse
            </tbody>
        </table>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function fetchResults() {
            ...
            if (excelBtn) {
                ...
                excelUrl.searchParams.set('resource', 'talent');
            ...
            if (pdfBtn) {
                ...
                pdfUrl.searchParams.set('resource', 'talent');
